...
let _dynamic_min = "/memory/dynamic-min"

(* mutable: written by domain manager *)

let _dynamic_max = "/memory/dynamic-max"
...
module Domain = struct
  ...
  let maxmem_of_dominfo di domain_type =
    let memory_max_kib =
      Xenctrl.pages_to_kib (Int64.of_nativeint di.Xenctrl.max_memory_pages)
    in
  ...
  let watch_xenstore () =
    Xenctrl.with_intf (fun xc ->
        ...
        (* Watches are generated by concurrent activity on the system. We must
           ...
           xenstored is more important for system reliability, we choose to
           drain its queue as quickly as possible and put the queue here. If
           this queue gets too large we should throw it away, disconnect and
           reconnect. *)
  ...
  let set_maxmem_noexn cnx domid target_kib =
    ...
    let maxmem_kib =
      xen_max_offset_kib (get_domain_type cnx domid) +* target_kib +* offset_kib
    in
  ...
  (** Query a domain's dynamic_min. Throws Xs_protocol.Enoent _ if the domain
      has been destroyed *)
  let get_dynamic_min cnx domid = Int64.of_string (read cnx domid _dynamic_min)

  (** Query a domain's dynamic_max. Throws Xs_protocol.Enoent _ if the domain
      has been destroyed *)
  let get_dynamic_max cnx domid = Int64.of_string (read cnx domid _dynamic_max)
end
...
let make_host ~verbose ~xc =
  (* Wait for any scrubbing so that we don't end up with no immediately usable
  ...
  (* Some VMs are considered by us (but not by xen) to have an
     "initial-reservation". For VMs which have never run (eg which are still
     being built or restored) we take the difference between memory_actual_kib
     and the reservation and subtract this manually from the host's free memory.
     Note that we don't get an atomic snapshot of system state so there is a
     natural race between the hypercalls. Hopefully the memory is being consumed
     fairly slowly and so the error is small. *)

  (* Additionally we have the concept of a 'reservation' separate from a domain
     which allows us to postpone domain creates until such time as there is lots
     of memory available. This minimises the chance that the remaining free
     memory will be too fragmented to actually use (some xen structures require
     contiguous frames) *)
  let reserved_kib = ref 0L in
  (* We cannot query simultaneously the host memory info and the domain memory
     info. Furthermore the call to domain_getinfolist is not atomic but
     comprised of many hypercall invocations. *)
  ...
  and scrub_pages_kib =
    Xenctrl.pages_to_kib (Int64.of_nativeint physinfo.Xenctrl.scrub_pages)
  and total_pages_kib =
